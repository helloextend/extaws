...
export class ExtAws {
  ...
  protected async handleMFA(authResponse: OktaAuthResponse, spinner?: Ora): Promise<void | never> {
    if (authResponse._embedded?.factors !== undefined && authResponse.stateToken !== undefined) {
      ...
      if (type === 'sms') {
        do {
          verify = await this.verifyFactor(
            { id: factor, stateToken: authResponse.stateToken },
            totpPrompt,
          )
          ...
          if (verify instanceof AxiosError) {
            spinner?.stop()
            throw new Error(verify.message)
          }
   ...
   */
  static async clearCredentials(options?: {
    config?: boolean
    credentials?: boolean
  }): Promise<boolean> {
   ...
   */
  async login(
    props?: { profile?: string; duration?: number; region?: string; role?: string },
    inputSpinner?: Ora,
    ...
    if (this.config.extaws.saveCreds)
      await ExtAws.localSecrets('SET', 'extaws', 'okta', credentials)
    ...
    if (props?.role) {
      ...
      if (searchResult !== undefined) {
        ...
        spinner.fail(
          `You do not have authorization to assume role (${props.role}). Please select from roles found`,
        )
   ...
   */
  static async localSecrets<T>(
    operation: 'GET' | 'SET' | 'DELETE',
    service: string,
    account: string,
    data?: T,
  ): Promise<boolean | void | null | string | T> {
   ...
   */
  protected async verifyFactor(
    factor: FactorType,
    mfaCode?: string | undefined,
  ): Promise<OktaAuthResponse | AxiosError> {
    try {
  ...
  private async parseRolesFromXML(): Promise<STSAssumeRole[]> {
    ...
    const roleText = this.document.findall(
      '*/saml2:AttributeStatement/saml2:Attribute/saml2:AttributeValue',
    )
  ...
  private async parseAssertionFromHtml(): Promise<void> {
    ...
    const handler = new htmlparser.DefaultHandler(function (err, dom) {
      ...
      if (samlObject.length == 0) {
        throw new Error(
          `SAML Object is empty. There are no valid roles for assertion. \nError Message: ${err}`,
        )
      }
